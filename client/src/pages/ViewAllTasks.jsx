...
import { QUERY_ALLTASKS, QUERY_TASKBYEMP } from '../utils/queries';
...
export default function ViewAllTasks(props) {
	const [query, setQuery] = React.useState(QUERY_TASKBYEMP);
	...
	const [tasks, setTasks] = React.useState(
		data?.tasks || data?.taskByEmp || []
	);

	React.useEffect(async () => {
		if (props.level === 2) {
			setQuery(QUERY_ALLTASKS);
			console.log('QUERY_ALLTASKS');
			await refetch();
			setTasks(data?.tasks || []);
		} else {
			setQuery(QUERY_TASKBYEMP);
			console.log('QUERY_TASKBYEMP');
			await refetch();
			setTasks(data?.taskByEmp || []);
		}
	}, [props.level, props._id]);
